...
define(['js/module.js', 'jquery', 'ngdialog'], function (controllers, $, ngdialog) {
    ...
    controllers.controller('controllercztx', function ($scope, resourceService, $filter, $http, $state, $rootScope, $localStorage, ngDialog, $stateParams,md5) {	         
     	$scope.isHeader=false;//是否显示头部
     	$scope.changeToggle = function(event){
            if ($(event.currentTarget).siblings("main").hasClass("new")) {   
            	$(event.currentTarget).siblings("main").slideUp('slow').removeClass("new");
    ...
    controllers.controller('controllertzcp', function ($scope, resourceService, $filter, $http, $state, $rootScope, $localStorage, ngDialog, $stateParams,md5) {	         
     	$scope.isHeader=false;//是否显示头部
     	$scope.changeToggle = function(event){
            if ($(event.currentTarget).siblings("main").hasClass("new")) {   
            	$(event.currentTarget).siblings("main").slideUp('slow').removeClass("new");
    ...
    controllers.controller('controllerhdjl', function ($scope, resourceService, $filter, $http, $state, $rootScope, $localStorage, ngDialog, $stateParams,md5) {	         
     	$scope.isHeader=false;//是否显示头部
     	$scope.changeToggle = function(event){
            if ($(event.currentTarget).siblings("main").hasClass("new")) {   
            	$(event.currentTarget).siblings("main").slideUp('slow').removeClass("new");
